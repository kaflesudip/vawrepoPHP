 ...
 * @property string $incident_image
 *
...
class VawhackappIncident extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('incident_title, incident_image', 'length', 'max'=>100),
			array('incident_source_url', 'length', 'max'=>200),
			array('death_number, injured_number', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, incident_title, incident_date, incident_time, incident_district_id, incident_source_id, incident_source_url, death_number, injured_number, verified, description, incident_image', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('incident_image',$this->incident_image,true);
